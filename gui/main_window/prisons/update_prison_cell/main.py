...
def update_prison_cell():
    UpdatePrisonCell()


class UpdatePrisonCell(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        ...
        entry_3 = Entry(
            self,
            font=("Montserrat Bold", 18 * -1),
            textvariable=self.data["charges"],
            foreground="#777777",
            bd=0,
            bg="#EFEFEF",
            highlightthickness=0,
    ...
    def initialize(self):
        self.selected_p_c_a_id = self.parent.selected_pca_id
        self.prison_cell_data = self.parent.prison_cell_data

        # Filter out all allocate prison for selected id prison_allocate
        self.selected_prison_cell_data = list(
            filter(lambda x: str(x[0]) == self.selected_p_c_a_id, self.prison_cell_data)
        )

        if self.selected_prison_cell_data:
            self.selected_prison_cell_data = self.selected_prison_cell_data[0]

            self.canvas.itemconfigure(self.id_text, text=self.selected_prison_cell_data[0])
            self.data["number"].set(self.selected_prison_cell_data[1])
            self.data["type"].set(self.selected_prison_cell_data[2])
            self.data["charges"].set(self.selected_prison_cell_data[3])

    def handle_update(self):
        result = db_controller.update_prison_cell(
            self.selected_p_c_a_id,
            prison_no=self.data["number"].get(),
            prison_type=self.data["type"].get(),
            charges=self.data["charges"].get(),
        )
