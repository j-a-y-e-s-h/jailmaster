...
def update_prisoners():
    UpdatePrisoners()


class UpdatePrisoners(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        Frame.__init__(self, parent, *args, **kwargs)
        self.parent = parent
        self.selected_p_c_a_id = self.parent.selected_rid
        ...
        entry_3 = Entry(
            self,
            font=("Montserrat Bold", 18 * -1),
            textvariable=self.data["Other Details"],
            foreground="#777777",
            bd=0,
            bg="#EFEFEF",
            highlightthickness=0,
    ...
    def initialize(self):
        self.selected_p_c_a_id = self.parent.selected_rid
        self.prisoner_s_data = self.parent.prisoners_data

        # Filter out all allocate prison for selected id prison_allocate
        self.selected_prisoners_data = list(
            filter(lambda x: str(x[0]) == self.selected_p_c_a_id, self.prisoner_s_data)
        )

        if self.selected_prisoners_data:
            self.selected_prisoners_data = self.selected_prisoners_data[0]

            self.canvas.itemconfigure(self.id_text, text=self.selected_prisoners_data[0])
            self.data["name"].set(self.selected_prisoners_data[1])
            self.data["address"].set(self.selected_prisoners_data[2])
            self.data["Other Details"].set(self.selected_prisoners_data[4])

    def handle_update(self):
        data = [
            x
            for x in [self.data[label].get() for label in ("name", "address", "Other Details")]
        ]

        result = db_controller.update_prisoners(
            name=data[0], address=data[1], other_details=data[2], id=self.selected_p_c_a_id
        )
        if result:
            messagebox.showinfo("Success", "Prisoner Updated")
            self.parent.navigate("view")
            self.parent.windows['view'].handle_refresh()
        else:
            messagebox.showerror("Error", "Failed to update Prisoner. Please Verify that all IDs are correct.")
