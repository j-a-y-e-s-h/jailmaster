...
def update_prison_allocate():
    UpdatePrisonAllocate()


class UpdatePrisonAllocate(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        Frame.__init__(self, parent, *args, **kwargs)
        self.parent = parent
        self.selected_p_c_a_id = self.parent.selected_pca_id
        ...
        self.data = {
            ...
            "allocate": StringVar(),
            "prison_id": StringVar(),
            "prison_allocate_date": StringVar(),
            "released": StringVar(),
        }
        ...
        entry_2 = Entry(
            ...
            font=("Montserrat Bold", 18 * -1),
            foreground="#777777",
            textvariable=self.data["Supplies"],
        )
        ...
        entry_5 = Entry(
            ...
            font=("Montserrat Bold", 18 * -1),
            foreground="#777777",
            textvariable=self.data["allocate"],
        )
        ...
        entry_6 = Entry(
            ...
            font=("Montserrat Bold", 18 * -1),
            foreground="#777777",
            textvariable=self.data["prison_allocate_date"],
        )
        ...
        entry_7 = Entry(
            ...
            font=("Montserrat Bold", 18 * -1),
            foreground="#777777",
            textvariable=self.data["prison_id"],
        )
        ...
        entry_8 = Entry(
            ...
            font=("Montserrat Bold", 18 * -1),
            foreground="#777777",
            textvariable=self.data["released"],
        )
    ...
    def initialize(self):
        ...
        # Filter out all allocate prison for selected id Prison Allocate
        self.selected_prison_allocate_data = list(
            filter(lambda x: str(x[0]) == self.selected_p_c_a_id, self.prison_allocate_data)
        )

        if self.selected_prison_allocate_data:
            self.selected_prison_allocate_data = self.selected_prison_allocate_data[0]

            self.canvas.itemconfigure(
                self.id_text, text=self.selected_prison_allocate_data[0]
            )
            self.data["p_id"].set(self.selected_prison_allocate_data[1])
            self.data["prison_id"].set(self.selected_prison_allocate_data[2])
            self.data["allocate"].set(self.selected_prison_allocate_data[3])
            self.data["released"].set(self.selected_prison_allocate_data[4])
            self.data["Supplies"].set(self.selected_prison_allocate_data[5])
            self.data["prison_allocate_date"].set(self.selected_prison_allocate_data[3])
    ...
    def handle_update(self):
        ...
        data = [
            ...
            for x in [
                self.data[label].get()
                for label in ("p_id", "allocate", "prison_id", "released", "Supplies")
            ]
        ]

        # Update data and show alert
        if db_controller.update_prison_allocate(self.selected_p_c_a_id, *data):
            messagebox.showinfo("Success", "Prison Allocate Updated Successfully")
            ...
            messagebox.showerror(
                "Error", "Error Updating Prison Allocate. Please check all ids exist"
            )
